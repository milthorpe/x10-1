...
final class FinishResilientPlace0 extends FinishResilient implements CustomSerialization {
    ...
    private transient var deferGlobalization:Boolean = false;
    ...
    private def globalInit() {
        ...
        if (!isGlobal) {
            ...
            if (deferGlobalization) {
                if (verbose>=1) debug(">>>> deferring globalization to following message for id="+id);
            } else {
                val gfs = this.ref;
                val myId = this.id;
    ...
    def notifyActivityCreation(srcPlace:Place, activity:Activity, kind:Int):Boolean {
        ...
        at (place0) @Immediate("notifyActivityCreation_to_zero") async {
            ...
            if (Place(srcId).isDead() || Place(dstId).isDead()) {
                ...
                try {
                    lock.lock();
                    states(myId).transitToLive(srcId, dstId, kind, "notifyActivityCreation");
                } finally {
                    lock.unlock();
                }
            }
        ...
        at (place0) @Immediate("notifyActivityCreationFailed_to_zero") async {
            if (Place(srcId).isDead() || Place(dstId).isDead()) {
                ...
                try {
                    ...
                    states(myId).transitToCompleted(srcId, dstId, kind, t);
                } finally {
                    lock.unlock();
                }
            }
            ...
            if (isGlobal) {
                ...
                at (place0) @Immediate("notifyActivityCreatedAndTerminated_quiescence_check_to_zero") async {
                    if (Place(srcId).isDead()) {
                        ...
                        try {
                            lock.lock();
                            states(myId).liveToCompleted(srcId, kind);
                        } finally {
                            lock.unlock();
                        }
                    }
        ...
        at (place0) @Immediate("notifyActivityCreatedAndTerminated_to_zero") async {
            if (Place(srcId).isDead() || Place(dstId).isDead()) {
                ...
                try {
                    ...
                    states(myId).transitToCompleted(srcId, dstId, kind, null);
                } finally {
                    lock.unlock();
                }
            }
        ...
        at (place0) @Immediate("notifyActivityTermination_to_zero") async {
            if (Place(dstId).isDead()) {
                ...
                try {
                    ...
                    states(myId).liveToCompleted(dstId, kind);
                } finally {
                    lock.unlock();
                }
            }
    ...
    def spawnRemoteActivity(place:Place, body:()=>void, prof:x10.xrx.Runtime.Profile):void {
        ...
        if (!isGlobal) {
            // globalize this activity as part of the spawn message to Place 0
            latch.lock(); // matching unlock at end of method
            deferGlobalization = true;
        }
            ...
            at (place0) @Immediate("spawnRemoteActivity_big_async_to_zero") async {
                ...
                try {
                    ...
                    at (wbgr) @Immediate("spawnRemoteActivity_big_back_to_spawner") async {
                        val fs = (gfs as GlobalRef[FinishResilientPlace0]{self.home == here})();
                        try {
        ...
        if (!isGlobal) {
            // state is now globalized.  must unlock
            isGlobal = true;
            latch.unlock();
        }
    ...
    private final def getParentId():Id {
